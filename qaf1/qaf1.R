...
gjr.m.model.estimation    = ugarchfit(spec=gjr.m.model.specification, data=dlAORD, solver = "solnp")
...
pdf("qaf1/qaf1.pdf", width=4*7, height=4*5)
plot(x=1:(321+h),y=c(dlAORD[8949:9269],rep(NA,h)), type="l", axes=FALSE, col=qaf3, ylab="", xlab="", lwd=4)
lines(x=1:321, y=as.vector(sigma(gjr.m.model.estimation))[8949:9269], col=qaf2,lwd=8)
for (i in 1:H){
...
img <- magick::image_read("qaf1/qaf1.pdf")
img |> magick::image_crop(geometry = "1800x1210+110+100")  -> img
# img
final_res<- hexSticker::sticker(img, package="qaf1", p_size=30,
                   p_family = "sans",
                   p_y = 1.5,
                   ...
                   s_width=2,
                   s_height = 1.1,
                   filename="qaf1/qaf1.png",h_fill="white",h_color = stickerColor)
